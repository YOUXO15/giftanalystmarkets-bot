...
- ежемесячная подписка: `3 TON` за `30` дней доступа
...
2. Бот создаёт счёт в Crypto Pay на `3 TON`.
3. Подписка активируется на `30` дней.
...
- `SUBSCRIPTION_INTRO_PRICE_TON` — legacy-переменная для совместимости; новые счета создаются по обычному тарифу
...
SUBSCRIPTION_INTRO_PRICE_TON=3
